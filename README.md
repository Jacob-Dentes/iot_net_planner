...
To introduce training data from other cities, start by going to [this Box folder](https://cornell.box.com/s/mh3mfpg478jld3hdprkx9js9hnbe921n) and downloading the precollected data from each desired city. Make sure that you download both the "X" and the "y" file for each city you would like to use. Place the downloaded files in a folder, **DO NOT CHANGE THEIR NAMES**. Then, add two additional arguments to the command in the previous paragraph. The first argument `folder_path` is a path to the folder containing the downloaded data files. The second argument `brooklyn,ithaca` is a comma-separated (no spaces) list of the cities to use premade training data from. The names must correspond exactly with the names of the data in the folder (for example, to use `brooklyn,ithaca` the folder must have all of the files `brooklyn_X.npy`, `brooklyn_y.npy`, `ithaca_X.npy`, and `ithaca_y.npy`). To train a model with only one city's data, just write that city (for example, `brooklyn` without any commas). It is recommended to try out several combinations of the extra cities with a testing set and comparing the Brier scores.
